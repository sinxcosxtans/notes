...
{	 
     //time_change();
   	 singletime[0]=TIME[5]/16;//年的十位
	 singletime[1]=TIME[5]%16;//年的个位
...
void main()
{
	...
	for(i=2;i<6;i++)//显示简易时钟四个字
	{
	 ...
	 while(1)
	 {
		...
		if(flag==1)key2();
		if(flag==1)key1();
		TIME[6]=CaculateWeekDay(2000+(TIME[5])/16*10+TIME[5]%16,TIME[4]/16*10+TIME[4]%16, TIME[3]/16*10+TIME[3]%16)+1;
